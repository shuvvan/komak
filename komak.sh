...
show_intro_logo() {
  ...
  BLACK_BG='\033[40m'  # پس‌زمینه سیاه

  # پیام اینترولوگو
  intro_message="KOMAK 3.2.1"
  term_width=$(tput cols)  # عرض ترمینال
  term_height=$(tput lines)  # ارتفاع ترمینال

  # موقعیت عمودی و افقی برای وسط‌چین کردن
  padding_left=$(( (term_width - ${#intro_message} - 4) / 2 ))
  padding_top=$(( (term_height / 2) - 1 ))

  # نمایش پیام اینترولوگو
  tput cup $padding_top $padding_left
  echo -e "${BLACK_BG}${WHITE}${BOLD}$(printf '%*s' "$padding_left" | tr ' ' ' ')${intro_message}${RESET}"
  sleep 2  # نمایش برای ۲ ثانیه
  clear
}
...
show_welcome_message() {
  ...
  # پیام خوش‌آمدگویی
  message="Welcome to Komak 3.2.1 Project!"
...
show_menu() {
  ...
  echo "4. Install Alireza Panel"
  echo "5. Install Sanaei Panel"
  echo -e "${RED}6. Exit (or press ESC)${RESET}"
  echo "------------------------------------------"
  
  read -p "Please select an option: " option
  case $option in
    1) update_upgrade ;;
    2) sudo ufw enable && echo "Firewall Enabled" ;;
    3) sudo ufw disable && echo "Firewall Disabled" ;;
    4) echo "Installing Alireza Panel..." ;;
    5) echo "Installing Sanaei Panel..." ;;
    6) exit ;;
